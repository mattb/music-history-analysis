...
def explore_taste_evolution(
    ...
    for year in range(start_year, end_year + 1):
        ...
        result["years"][year] = {
            ...
            "top_discoveries": discovered.head(5)["artist"].tolist()
            if len(discovered) > 0
            else [],
...
def find_musical_bridges(
    ...
    user_set = {
        crossref.normalize_for_matching(x["artist"]) for x in result["user_similar"]
    }
    critics_set = {
        crossref.normalize_for_matching(x["artist"]) for x in result["critics_similar"]
    }
...
def _analyze_single_artist(state: AnalysisState, df: pd.DataFrame, artist: str) -> dict:
    ...
    if state.critics_embeddings:
        ...
        if norm in state.critics_embeddings.artist_to_idx:
            critics_similar = [
                {"artist": name, "similarity": round(score, 3)}
                for name, score in state.critics_embeddings.find_similar(
                    actual_name, top_n=10
                )
            ]
    ...
    for year in state.get_all_critics_years():
        ...
        for critic_list in critics_data:
            critic = critic_list.get("critic", "Unknown")
            for album in critic_list.get("albums", []):
                if crossref.normalize_for_matching(
                    album.get("artist", "")
                ) == crossref.normalize_for_matching(actual_name):
    ...
    return {
        ...
        "years_active": (last_play.year - first_play.year + 1)
...
def find_similar_artists(
    ...
    if source == "user":
        if (
            not state.user_embeddings
            or artist not in state.user_embeddings.artist_to_idx
        ):
            return []
    ...
    return to_serializable(
        [{"artist": name, "similarity": round(score, 3)} for name, score in similar]
    )
...
def get_temporal_patterns(state: AnalysisState, year: int | None = None) -> dict:
    ...
    weekdays = {
        day: {
            "plays": int(weekday_counts.get(day, 0)),
            "pct": round(weekday_counts.get(day, 0) / total * 100, 1),
        }
    ...
    months = {
        month_names[m - 1]: {
            "plays": int(month_counts.get(m, 0)),
            "pct": round(month_counts.get(m, 0) / total * 100, 1),
        }
        for m in range(1, 13)
    }
...
def _get_single_year_review(
    state: AnalysisState, df_full: pd.DataFrame, year: int
) -> dict:
    ...
    for _, row in top_albums_df.iterrows():
        ...
        plays = row["plays"]
        album_df = df_full[
            (df_full["artist"] == artist_name) & (df_full["album"] == album_name)
        ]
        ...
        result["discoveries"].append(
            {
                ...
                "first_date": row["timestamp"].isoformat()
    ...
    if critics_data:
        ...
        result["critics"] = {
            ...
            "your_overlap_pct": round(matched_count / total_critics_albums * 100, 1)
            if total_critics_albums > 0
            else 0,
    ...
    if db_stats and db_stats.get("has_full_schema"):
        try:
            ...
            album_plays = (
                df_albums.groupby(["artist", "album"]).size().reset_index(name="plays")
            )
            ...
            if albums_matched > 0:
                ...
                result["metadata"] = {
                    ...
                    "genres": [
                        {
                            "name": g,
                            "plays": p,
                            "pct": round(p / total_genre_plays * 100, 1),
                        }
                    ...
                    "labels": [
                        {
                            "name": label,
                            "plays": plays,
                            "pct": round(plays / total_label_plays * 100, 1),
                        }
                        for label, plays in sorted_labels[:10]
                    ],
                    "countries": [
                        {
                            "code": c,
                    ...
                    "new_vs_catalog": {
                        "new_pct": round(new_releases / total_matched_plays * 100, 1)
                        if total_matched_plays > 0
                        else 0,
                        "catalog_pct": round(
                            catalog_releases / total_matched_plays * 100, 1
                        )
                        if total_matched_plays > 0
                        else 0,
                    },
...
def get_listening_by_release_era(
    ...
    if not db_stats or not db_stats.get("has_full_schema"):
        return {
            "error": "MusicBrainz database not available. Run: lastfm metadata download"
        }
...
def get_common_transitions(state: AnalysisState, artist: str, top_n: int = 10) -> dict:
    ...
    try:
        ...
        artist_indices = df_sorted[
            df_sorted["artist"].str.lower() == artist_lower
        ].index.tolist()
        ...
        return to_serializable(
            {
                "artist": actual_name,
                "total_plays": total_plays,
                "plays_before": [{"artist": a, "count": c} for a, c in top_before],
                "plays_after": [{"artist": a, "count": c} for a, c in top_after],
            }
        )
    except Exception as e:
        return {
            "error": f"Exception in get_common_transitions: {type(e).__name__}: {str(e)}"
        }
...
def get_discovery_context(state: AnalysisState, artist: str) -> dict:
    ...
    try:
        ...
        before_plays = df[
            (df["timestamp"] >= day_before) & (df["timestamp"] < first_timestamp)
        ]
        after_plays = df[
            (df["timestamp"] > first_timestamp) & (df["timestamp"] <= day_after)
        ]

        # Get unique artists from before/after (excluding the discovered artist)
        artists_before = (
            before_plays[before_plays["artist"].str.lower() != artist_lower]["artist"]
            .value_counts()
            .head(5)
        )
        artists_after = (
            after_plays[after_plays["artist"].str.lower() != artist_lower]["artist"]
            .value_counts()
            .head(5)
        )
        ...
        if len(session_matches) > 0:
            ...
            session_artists = (
                session_plays[session_plays["artist"].str.lower() != artist_lower][
                    "artist"
                ]
                .unique()
                .tolist()
        ...
        return {
            "error": f"Exception in get_discovery_context: {type(e).__name__}: {str(e)}"
        }
...
def explore_critics_world(state: AnalysisState, year: Optional[int] = None) -> dict:
    ...
    for key, album_data in album_index.items():
        ...
        for c in critics_list:
            ...
            critic_info = next(
                (x for x in critic_alignment if x["name"] == c["critic"]), None
            )
            if critic_info:
    ...
    return to_serializable(
        {
            "summary": {
                ...
                "years_covered": years,
                "your_overall_alignment": round(you_heard / total_acclaimed * 100, 1)
                if total_acclaimed > 0
                else 0,
...
def get_album_acclaim(
    state: AnalysisState, artist: str, album: str, year: Optional[int] = None
) -> dict:
    ...
    critics_list_sorted = sorted(
        critics_list, key=lambda x: (x["year"], x["rank"] or 999)
    )
    ...
    if in_library:
        ...
        if len(album_plays) > 0:
            ...
            # Find peak month
            monthly = album_plays.groupby(
                album_plays["timestamp"].dt.to_period("M")
            ).size()
            ...
            familiarity = familiarity_scores.get(
                (album_data["artist"], album_data["album"]), 0
            )
...
def get_my_validated_albums(
    state: AnalysisState, year: Optional[int] = None, limit: int = 50
) -> dict:
    ...
    if validated:
        ...
        most_validated_artist = (
            max(artist_counts.items(), key=lambda x: x[1])[0] if artist_counts else None
        )
        ...
        for v in validated:
            for y in v["critics_years"]:
                year_matches[y] += 1
        most_aligned_year = (
            max(year_matches.items(), key=lambda x: x[1])[0] if year_matches else None
        )
    else:
        avg_critics = 0
...
def get_critic_profile(
    state: AnalysisState, critic_name: str, year: Optional[int] = None
) -> dict:
    ...
    if critic_key is None:
        return {
            "error": f"Critic '{critic_name}' not found. Try searching with a partial name."
        }
    ...
    for name, cdata in critic_index.items():
        ...
        overlap = sum(
            ...
            for p in cpicks
            if (
                crossref.normalize_for_matching(p["artist"]),
                crossref.normalize_for_matching(p["album"]),
            )
            in listened_albums
        )
    ...
    rank = next(
        (
            i + 1
            for i, (name, _) in enumerate(all_critics_alignment)
            if name == critic_key
        ),
        None,
    )
    ...
    return to_serializable(
        {
            ...
            "alignment_with_you": {
                "score": round(len(heard) / (len(heard) + len(missed)), 3)
                if (heard or missed)
                else 0,
...
def search_critics_for_artist(
    state: AnalysisState, artist: str, year: Optional[int] = None
) -> dict:
...
def get_discovered_artists(state: AnalysisState, year: int) -> list:
    ...
    return to_serializable(
        [
            {
                "artist": row["artist"],
                "first_play": row["first_play"].isoformat()
                if hasattr(row["first_play"], "isoformat")
                else str(row["first_play"]),
